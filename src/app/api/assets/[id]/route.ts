...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try { const filePath = resolveLocalObjectPath(asset.objectKey); await stat(filePath); const stream = createReadStream(filePath); return new NextResponse(Readable.toWeb(stream) as ReadableStream, { headers: { "Content-Type": asset.mimeType, "Content-Disposition": `${asset.mimeType.startsWith("image/") ? "inline" : "attachment"}; filename*=UTF-8''${encodeURIComponent(asset.fileName)}`, "Cache-Control": asset.visibility === "PUBLIC" ? "public, max-age=3600" : "private, no-store", "X-Content-Type-Options": "nosniff" } }); } catch { return new NextResponse("File unavailable", { status: 404 }); }
}
