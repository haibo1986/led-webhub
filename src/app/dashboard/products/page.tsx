...
import { translateProductAction } from "./actions";
...
export default async function ProductsPage({ searchParams }: { searchParams: Promise<{ deleted?: string }> }) {
  ...
  const aiEnabled = (await getDb().tenant.findUnique({ where: { id: session.tenantId }, select: { aiEnabled: true } }))?.aiEnabled ?? false; const products = await getDb().product.findMany({ where: { tenantId: session.tenantId, deletedAt: null }, orderBy: { updatedAt: "desc" }, include: { category: { select: { nameZh: true } }, translations: { select: { locale: true, name: true, isPublished: true } }, _count: { select: { variants: true, assets: true } } } });
