...
export async function createProjectAction(formData: FormData) {
  ...
  let item;
  try {
    item = await getDb().projectCase.create({ data: { tenantId: session.tenantId, slug: d.slug, category: d.category, location: d.location || null, translations: { create: [{ locale: "ZH_CN", title: d.titleZh, summary: d.summaryZh, description: d.descriptionZh }, { locale: "EN", title: d.titleEn, summary: d.summaryEn, description: d.descriptionEn }] } } });
  } catch (error) {
    if ((error as { code?: string }).code === "P2002") redirect("/dashboard/projects/new?error=duplicate");
    throw error;
  }
  await getDb().auditLog.create({ data: { tenantId: session.tenantId, actorId: session.userId, action: "PROJECT_CREATED", resource: "ProjectCase", resourceId: item.id } });
  redirect(`/dashboard/projects/${item.id}?created=1`);
}
...
export async function updateProjectAction(id: string, formData: FormData) {
  ...
  const parsed = parse(formData);
  if (!parsed.success) redirect(`/dashboard/projects/${parsedId.data}?error=invalid`);
