...
export default async function InquiriesPage({ searchParams }: { searchParams: Promise<{ page?: string; status?: string }> }) {
  ...
  const [items, total] = await Promise.all([
    getDb().inquiry.findMany({ where, orderBy: { createdAt: "desc" }, skip: (page - 1) * PAGE_SIZE, take: PAGE_SIZE, include: { emailLogs: { orderBy: { createdAt: "desc" }, take: 1 }, notes: { orderBy: { createdAt: "desc" }, take: 3, include: { author: { select: { name: true } } } } } }),
