import { CheckCircle2, ChevronDown, ChevronUp, Download, Globe, Languages, LayoutTemplate, Menu, Palette, Plus, Save, Search, Trash2 } from "lucide-react";
...
export default async function SettingsPage({ searchParams }: { searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  const tenant = await getDb().tenant.findUniqueOrThrow({
    ...
    select: { name: true, shortName: true, description: true, email: true, phone: true, address: true, primaryColor: true, secondaryColor: true, publicDownloads: true, navigationConfig: true, homepageConfig: true, seoConfig: true, domains: { orderBy: { createdAt: "asc" } } },
  });
