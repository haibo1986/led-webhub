...
export default async function ProjectDetail({ params }: PageProps<"/[locale]/projects/[slug]">) {
  ...
  const item = await getDb().projectCase.findFirst({ where: { tenantId: tenant.id, slug, status: "PUBLISHED", deletedAt: null, translations: { some: { locale: dbLocale, isPublished: true } } }, include: { translations: { where: { locale: dbLocale, isPublished: true }, take: 1 }, products: { orderBy: { sortOrder: "asc" }, include: { product: { select: { slug: true, model: true, status: true, deletedAt: true, translations: { where: { locale: dbLocale, isPublished: true }, take: 1, select: { name: true } } } } } } } });
