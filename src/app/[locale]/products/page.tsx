...
export default async function ProductIndex({ params, searchParams }: PageProps<"/[locale]/products">) {
  ...
  const [products, categories, total] = await Promise.all([
    getDb().product.findMany({
      ...
      take: PAGE_SIZE,
      include: { category: true, translations: { where: { locale: dbLocale, isPublished: true }, take: 1 }, variants: { orderBy: { sortOrder: "asc" }, take: 3 }, _count: { select: { assets: true, variants: { where: { isActive: true } } } } },
    }),
  ...
  return <main className="catalog-page">
    ...
    <div className="catalog-grid">{products.map(product => <Link href={`/${locale}/products/${product.slug}`} key={product.id}><div className="catalog-art"><i /><span>{product.model}</span></div><p>{zh ? product.category.nameZh : (product.category.nameEn ?? product.category.nameZh)}</p><h2>{product.translations[0].name}</h2><div><span><Layers3 />{product._count.variants} SKU</span><span>{tenant.publicDownloads ? product._count.assets : 0} {zh ? "份资料" : "files"}</span></div><b>{zh ? "查看完整规格" : "View specifications"}<ArrowRight /></b></Link>)}{!products.length && <p className="public-empty">{q ? (zh ? `没有匹配「${q}」的产品。` : `No products match "${q}".`) : (zh ? "该分类暂无已发布产品。" : "No published products in this category.")}</p>}</div>
